...
inline image* load_image( const wchar_t* file_path, bool vflip=true, bool force_rgb=true, bool use_cache=false ){ static decltype(&load_image) rex_load_image = (decltype(&load_image)) GetProcAddress(GetModuleHandleW(nullptr),"rex_load_image"); return rex_load_image( file_path, vflip, force_rgb, use_cache ); }
inline image* load_image_header( const wchar_t* file_path ){ static decltype(&load_image_header) rex_load_image_header = (decltype(&load_image_header)) GetProcAddress(GetModuleHandleW(nullptr),"rex_load_image_header"); return rex_load_image_header( file_path ); }
...
inline void   save_image( const wchar_t* file_path, image* pimage, bool vflip=true, bool force_rgb=false, int quality=95 ){ static decltype(&save_image) rex_save_image = (decltype(&save_image)) GetProcAddress(GetModuleHandleW(nullptr),"rex_save_image"); if(pimage) rex_save_image( file_path, pimage, vflip, force_rgb, quality ); }
