...
inline const wchar_t* atow( const char* a ){ uint cp=is_utf8(a)?CP_UTF8:0; int l=MultiByteToWideChar(cp,0,a,-1,0,0);wchar_t* w=__strbuf<wchar_t>(l); MultiByteToWideChar(cp,0,a,-1,w,l);return w; }
inline const char* wtoa( const wchar_t* w ){ int l=WideCharToMultiByte(CP_GXUT,0,w,-1,0,0,0,0);char* a=__strbuf(l);WideCharToMultiByte(CP_GXUT,0,w,-1,a,l,0,0);return a; }
inline const char* atoa( const char* src, uint src_cp, uint dst_cp=CP_GXUT ){ if(src_cp==dst_cp) return __strdup(src); int l=MultiByteToWideChar(src_cp,0,src,-1,0,0);wchar_t* w=__strbuf<wchar_t>(l); MultiByteToWideChar(src_cp,0,src,-1,w,l); l=WideCharToMultiByte(dst_cp,0,w,-1,0,0,0,0);char* a=__strbuf(l);WideCharToMultiByte(dst_cp,0,w,-1,a,l,0,0);return a; }
#else
