...
struct frustum_t : public array<vec4, 6> // left, right, top, bottom, near, far
{
	...
	__forceinline frustum_t& update( const mat4& view_projection_matrix ){ auto r=reinterpret_cast<const array<vec4,4>&>(view_projection_matrix); auto* p=data();for(int k=0;k<6;k++){ p[k]=r[3]+r[k>>1]*float(1-(k&1)*2);p[k]/=p[k].xyz.length();} return *this; }
...
struct camera_t
{
	...
	vec3 eye; float width=0; vec3 at; float fn; alignas(16) vec3 up, dir; // 16-bytes aligned for std140 layout; eye.a=width, center.a=fn, dir=at-eye
