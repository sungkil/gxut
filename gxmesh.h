...
struct bvh_t : public acc_t // two-level hierarchy: mesh or geometry
{
	...
	virtual void* node_ptr( size_t index ) override { return nodes.empty()?nullptr:((&nodes[0])+index); }
};
...
inline void* geometry::acc_begin() const { if(!root->acc) return nullptr; return root->acc->node_ptr(acc_first_index); }
inline void* geometry::acc_end() const { if(!root->acc) return nullptr; return root->acc->node_ptr(acc_first_index+acc_count); }
