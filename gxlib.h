...
// early string function for path
...
// path queries
inline bool path_exists( const char* path ){ return path&&*path&&access(path,0)==0; }
...
inline bool is_unc_path( const char* path ){ if(!path||!*path) return false; return (path[0]=='\\'&&path[1]=='\\')||(path[0]=='/'&&path[1]=='/'); }
