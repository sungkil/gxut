...
// platform-specific header files and configurations
	...
	inline HANDLE& safe_close( HANDLE& h ){ if(h&&h!=INVALID_HANDLE_VALUE) CloseHandle(h); return h=INVALID_HANDLE_VALUE; }
	template <class T> concept __has_Release_c = requires(T* p) { p->Release(); };
	template <class T> concept __has_release_c = requires(T* p) { p->release(); };
	template <class T> concept releasable_c = __has_Release_c<T> || __has_release_c<T>;
	template <releasable_c T> T*& safe_release( T*& p ){ if(!p) return p; if constexpr(__has_Release_c<T>) p->Release(); else if constexpr(__has_release_c<T>) p->release(); return p=nullptr; }
	template <releasable_c T> void safe_release( T& p ){ if constexpr(__has_Release_c<T>) p.Release(); else if constexpr(__has_release_c<T>) p.release(); }
	template <releasable_c T> void safe_release( std::vector<T*>& v ){ for( auto*& p:v ) safe_release(p); }
	template <releasable_c T> struct auto_release_ptr_t { T* ptr=nullptr; ~auto_release_ptr_t(){ safe_release(ptr); } auto_release_ptr_t()=default; auto_release_ptr_t( T* p ){ ptr=p; } operator T*(){ return ptr; } operator const T*() const { return ptr; } T* operator->(){ return ptr; } const T* operator->() const { return ptr; } };
...
#else
inline const wchar_t* atow( const char* a ){ if(!a) return nullptr; if(!*a) return L""; const char* p=a; size_t l=mbsrtowcs(0,&p,0,0); wchar_t* b=__strbuf<wchar_t>(l); mbstate_t s={}; mbsrtowcs(b,&p,l+1,&s); b[l+1]=0; return b; }
inline const char* wtoa( const wchar_t* w ){ if(!w) return nullptr; if(!*w) return ""; const wchar_t* p=w; size_t l=wcsrtombs(0,&p,0,0); char* b=__strbuf(l); mbstate_t s={}; wcsrtombs(b,&p,l+1,&s); b[l+1]=0; return b; }
#endif
...
struct path_t
{
	...
	path_t( string_view_type s ) noexcept : path_t() { size_t l=s.size(); if(l) memcpy(_data,s.data(),l*sizeof(value_type)); _data[l]=0; }
...
#define build_msg() va_list a;va_start(a,fmt);auto* msg=vformat(fmt,a);va_end(a)
struct mbox_t
{
	HWND& owner(){ return _owner; }
	mbox_t owner( HWND hwnd ){ mbox_t t=*this; if(hwnd) t._owner=hwnd; return t; }
	mbox_t title( const char* title ){ mbox_t t=*this; if(title) t._title=title; return t; }
	void warning( __printf_format_string__ const char* fmt, ... ){ build_msg(); MessageBoxA(_owner,msg,format("%s: warning",_title.c_str()),MB_OK|MB_ICONWARNING|MB_SYSTEMMODAL); }
	bool error( __printf_format_string__ const char* fmt, ... ){ build_msg(); MessageBoxA(_owner,msg,("%s: error",_title.c_str()),MB_OK|MB_ICONERROR|MB_SYSTEMMODAL); return false; }
	bool confirm( __printf_format_string__ const char* fmt, ... ){ build_msg(); return IDYES==MessageBoxA(_owner,msg,_title.c_str(),MB_YESNO|MB_ICONQUESTION|MB_SYSTEMMODAL); }
	void operator()( __printf_format_string__ const char* fmt, ... ){ build_msg(); MessageBoxA(_owner,msg,_title.c_str(),MB_OK|MB_ICONINFORMATION|MB_SYSTEMMODAL); }
protected:
	HWND	_owner=nullptr;
	string	_title=exe::name();
};}
inline os::mbox_t mbox;
__noinline bool ebox( __printf_format_string__ const char* fmt, ... ){ build_msg(); return mbox.error(msg); }
#undef build_msg
namespace os {
...
struct image_header : public image
{
	image_header() = default;
	image_header( int width, int height, int depth=8, int channels=3, unsigned align=4, void* data=nullptr ){ this->width=width; this->height=height; this->depth=depth; this->channels=channels; const_cast<unsigned&>(this->align)=align; if(data) data=(decltype(data))data; }
	~image_header(){ data=nullptr; }
};
