...
__noinline bool updater::fetch()
{
	...
	path server_name;
	for( auto& t : {name, name.replace_extension("7z"), name.replace_extension("zip"), name+".7z", name+".zip"} )
	{
	...
	if(b_zip) // try to extract the app from zip/7z
	{
		...
		{
			path d1=cache.dst.dir()+e.name; if(d1.exists()&&FileTimeToTime(e.mtime)<=d1.mtime()) continue;
			if(!z->extract_to_files(cache.dst.dir(),wtoa(e.name))){ printf( "[update] unable to extract %s\n", cache.dst.name() ); safe_delete(z); return false; }
		}
		safe_delete(z);
	}
...
__noinline bool update( string name="", path dst_path="" )
{
	if(__argc>2&&wcscmp(__wargv[1],L"--update")==0)
	{
		...
		int k=0,kn=1024; for(;k<kn;k++){ if(os::find_process(dst1.c_str()).empty())break; Sleep(10); } // wait for dst1 closes
